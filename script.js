cat > /mnt/user-data/outputs/script.js << 'SCRIPTEOF'
// ============================================================
...
function drawClasico() {
  ...
  chartClasico = new Chart(document.getElementById('chart-clasico'), {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Nivel de inventario',       data,     borderColor: '#378ADD', backgroundColor: 'rgba(55,138,221,0.08)', fill: true,  pointRadius: 0, borderWidth: 2,   tension: 0 },
        { label: 'Inventario promedio (Q/2)', data: avgLine, borderColor: '#888780', borderDash: [6,4], borderWidth: 1.5, pointRadius: 0, fill: false, tension: 0 }
      ]
    },
...
function drawEstocastico() {
  ...
  chartEsto = new Chart(document.getElementById('chart-esto'), {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Nivel de inventario',    data,                             borderColor: '#378ADD', backgroundColor: 'rgba(55,138,221,0.07)', fill: true,  pointRadius: 0, borderWidth: 2,   tension: 0.3 },
        { label: 'Punto de reorden (ROP)', data: data.map(() => ROP),        borderColor: '#D85A30', borderDash: [6,4], borderWidth: 1.5, pointRadius: 0, fill: false, tension: 0 },
        { label: 'Stock de seguridad',     data: data.map(() => SS),         borderColor: '#1D9E75', borderDash: [3,3], borderWidth: 1.5, pointRadius: 0, fill: false, tension: 0 }
      ]
    },
    options: chartOptions('Semanas', 'Inventario (kg)')
  });
...
function drawComparativo() {
  const idx = +document.getElementById('sel-comp').value;
...
function chartOptions(xLabel, yLabel) {
  return {
    ...
    plugins: {
      ...
      tooltip: { callbacks: { label: ctx => ctx.dataset.label + ': ' + ctx.parsed.y.toFixed(2) + ' kg' } }
    },
    scales: {
      x: {
